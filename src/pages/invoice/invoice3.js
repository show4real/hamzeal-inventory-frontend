            ...
            <table class="items" style={{ marginTop: 30, marginLeft:17 }}>
              <thead style={{ }}>
                <tr>
                  ...
                  <th class="heading description" colSpan={2} style={{ paddingRight:120, textAlign:'right', color: 'black', fontFamily: 'monaco, Consolas, Lucida Console, monospace', fontSize: 20, fontWeight: 700 }}>Description</th>
                  <th class="heading qty" style={{ color: 'black', fontFamily: 'monaco, Consolas, Lucida Console, monospace', paddingRight:92, textAlign:'right', fontSize: 20, fontWeight: 700 }}>Qty</th>
                  <th class="heading rate" style={{ color: 'black', fontFamily: 'monaco, Consolas, Lucida Console, monospace', paddingRight:92, textAlign:'right', fontSize: 20, fontWeight: 700 }}>Rate</th>
                  <th class="heading amount" style={{ color: 'black', fontFamily: 'monaco, Consolas, Lucida Console, monospace',paddingRight:92, fontSize: 20, fontWeight: 700 }}>Cost</th>
                </tr>
              </thead>
                ...
                {Object.keys(items).length !== 0 ? items.map((item, key) => {
                  ...
                  return (
                    <tr style={{}}>
                      ...
                      <td colSpan={2} ><div class="row-spacing" style={{paddingRight: 120,textAlign:'right', fontSize: 20, width: '80mm', lineHeight: 1.5, }}>{item.description}</div></td>
                      <td style={{ verticalAlign: 'top' }}><div class="row-spacing" style={{paddingRight: 92, textAlign:'right',fontSize: 20, paddingTop: 0 }}>{item.quantity}</div></td>
                      <td style={{ verticalAlign: 'top' }}><div class="row-spacing" style={{paddingRight: 92, fontSize: 20,textAlign:'right', paddingTop: 0 }}>{this.formatCurrency2(item.rate)}</div></td>
                      <td style={{ verticalAlign: 'top' }}><div class="row-spacing" style={{ paddingRight: 92, fontSize: 20, textAlign:'right', paddingTop: 0 }}>{this.formatCurrency2(item.amount)}</div></td>
                    </tr>
                 ...
                 {Object.keys(pos_items).length !== 0 ? pos_items.map((item, key) => {
                      ...
                      return (
                        <tr style={{}}>
                          ...
                          <td colSpan={2} ><div class="row-spacing" style={{paddingRight: 120, whiteSpace: 'pre-line', fontSize: 20, width: '80mm', lineHeight: 1.5, }}>{item.order.product_name}</div></td>
                          <td style={{ verticalAlign: 'top' }}><div class="row-spacing" style={{paddingRight: 92, fontSize: 20, paddingTop: 0, textAlign:'right'}}>{item.qty_sold}</div></td>
                          <td style={{ verticalAlign: 'top' }}><div class="row-spacing" style={{paddingRight: 92,fontSize: 20, paddingTop: 0,textAlign:'right' }}>{this.formatCurrency2(item.selling_price)}</div></td>
                          <td style={{ verticalAlign: 'top' }}><div class="row-spacing" style={{ paddingRight: 92,fontSize: 20, paddingTop: 0, textAlign:'right' }}>{this.formatCurrency2(item.selling_price*item.qty_sold)}</div></td>
                        </tr>
                ...
                <tr>
                  ...
                  <td style={{ paddingRight: 92, }}>
                    <tr style={{ paddingTop: 20 }}>
                      <td style={{ fontSize: 20, textAlign: 'right', fontFamily: 'monaco, Consolas, Lucida Console, monospace', }}
                      >Total:{this.formatCurrency(invoice.currency, invoice.amount)}</td>
                    ...
                    <tr style={{ paddingTop: 10 }}>
                      <td style={{ fontSize: 20, textAlign: 'right', fontFamily: 'monaco, Consolas, Lucida Console, monospace', }}
                      >Paid:{this.formatCurrency(invoice.currency, invoice.total_payment)}</td>


                    </tr>
                    <tr style={{ paddingTop: 10 }}>
                      {invoice.total_balance > 0 && <>
                        <td style={{ fontSize: 20, textAlign: 'right', fontFamily: 'monaco, Consolas, Lucida Console, monospace', }}>Balance:{this.formatCurrency(invoice.currency, invoice.total_balance)}</td>
                      </>}
                    </tr>
                  </td>
                </tr>
